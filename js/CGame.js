   ...
   this.afterRollDice = function(){
    ...
    this.cpuChoicePiece = function(){
        ...
        if (_aBar[BAR_DOWN].getNumPieces() > 0) {
            ...
            if (_bDoubleDice){                
                var aSingleMove = [ aMoves[0][0] ];
                if(aMoves[0].length>1 && _aBar[BAR_DOWN].getNumPieces()>1){
                    //REMOVE ALL OTHER SUM MOVES, LET CPU DECIDE ONE BY ONE
                    aMoves[0] = aSingleMove;
                }
            }else{
                if ( (aMoves[2].length>0&&_aBar[BAR_DOWN].getNumPieces()>1) ){
                    //CAN'T USE DOUBLE DICE FOR 1 PIECE BECOUSE THERE ARE MORE PIECE TO QUIT THE BAR
                    aMoves[2] = new Array();
                }
            }
